...
struct SectorInfo {
  ...
  uint8_t ubInvestigativeState;  // When the sector is attacked by the player, the state increases by
  ...
  int8_t bLastKnownEnemies;  // -1 means never been there, no idea, otherwise it's what we'd observed
  ...
  uint8_t ubTravelRating;  // Represents how travelled a sector is.  Typically, the higher the travel
