...
void WWWRegionMvtCallback(struct MOUSE_REGION *pRegion, INT32 iReason) {
  ...
  if (iReason & MSYS_CALLBACK_REASON_LOST_MOUSE) {
    ...
    BltVideoObject(vsFB, hLapTopIconHandle, (UINT16)iFrame, LAPTOP_ICONS_X, LAPTOP_ICONS_WWW_Y,
...
void EmailRegionMvtCallback(struct MOUSE_REGION *pRegion, INT32 iReason) {
  ...
  if (iReason & MSYS_CALLBACK_REASON_LOST_MOUSE) {
    ...
    BltVideoObject(vsFB, hLapTopIconHandle, (UINT16)iFrame, LAPTOP_ICONS_X, LAPTOP_ICONS_MAIL_Y,
                   VO_BLT_SRCTRANSPARENCY);
    if (fUnReadMailFlag) {
      GetVideoObject(&hLapTopIconHandle, guiUNREAD);
      BltVideoObject(vsFB, hLapTopIconHandle, 0, LAPTOP_ICONS_X + CHECK_X,
                     LAPTOP_ICONS_MAIL_Y + CHECK_Y, VO_BLT_SRCTRANSPARENCY);
    }
    DrawLapTopText();
    HighLightRegion(giCurrentRegion);
    InvalidateRegion(0, 0, 640, 480);
...
void FinancialRegionMvtCallback(struct MOUSE_REGION *pRegion, INT32 iReason) {
  ...
  if (iReason & MSYS_CALLBACK_REASON_LOST_MOUSE) {
    ...
    BltVideoObject(vsFB, hLapTopIconHandle, (UINT16)iFrame, LAPTOP_ICONS_X - 4,
                   LAPTOP_ICONS_FINANCIAL_Y, VO_BLT_SRCTRANSPARENCY);
    DrawLapTopText();
    HighLightRegion(giCurrentRegion);
    InvalidateRegion(0, 0, 640, 480);
  }
...
void HistoryRegionMvtCallback(struct MOUSE_REGION *pRegion, INT32 iReason) {
  ...
  if (iReason & MSYS_CALLBACK_REASON_LOST_MOUSE) {
    ...
    BltVideoObject(vsFB, hLapTopIconHandle, (UINT16)iFrame, LAPTOP_ICONS_X, LAPTOP_ICONS_HISTORY_Y,
                   VO_BLT_SRCTRANSPARENCY);
    DrawLapTopText();
    HighLightRegion(giCurrentRegion);
    InvalidateRegion(0, 0, 640, 480);
...
void FilesRegionMvtCallback(struct MOUSE_REGION *pRegion, INT32 iReason) {
  ...
  if (iReason & MSYS_CALLBACK_REASON_LOST_MOUSE) {
    ...
    BltVideoObject(vsFB, hLapTopIconHandle, (UINT16)iFrame, LAPTOP_ICONS_X,
                   LAPTOP_ICONS_FILES_Y + 7, VO_BLT_SRCTRANSPARENCY);
    DrawLapTopText();
    HighLightRegion(giCurrentRegion);
    InvalidateRegion(0, 0, 640, 480);
  }
...
void PersonnelRegionMvtCallback(struct MOUSE_REGION *pRegion, INT32 iReason) {
  ...
  if (iReason & MSYS_CALLBACK_REASON_LOST_MOUSE) {
    ...
    BltVideoObject(vsFB, hLapTopIconHandle, (UINT16)iFrame, LAPTOP_ICONS_X,
                   LAPTOP_ICONS_PERSONNEL_Y, VO_BLT_SRCTRANSPARENCY);
    DrawLapTopText();
    HighLightRegion(giCurrentRegion);
    InvalidateRegion(0, 0, 640, 480);
  }
...
BOOLEAN InitTitleBarMaximizeGraphics(UINT32 uiBackgroundGraphic, STR16 pTitle, UINT32 uiIconGraphic,
  ...
  BltVideoObject(GetVSByID(guiTitleBarSurface), hImageHandle, 0, 0, 0, VO_BLT_SRCTRANSPARENCY);

  // blit th icon onto the tool bar
  GetVideoObject(&hImageHandle, uiIconGraphic);
  BltVideoObject(GetVSByID(guiTitleBarSurface), hImageHandle, usIconGraphicIndex,
...
BOOLEAN DisplayTitleBarMaximizeGraphic(BOOLEAN fForward, BOOLEAN fInit, UINT16 usTopLeftX,
  ...
  if (fForward) {
    ...
    if (ubCount > 1) {
      ...
      VSurfaceBlitBufToBuf(vsSB, vsFB, (UINT16)LastRect.iLeft, (UINT16)LastRect.iTop, sWidth,
                           sHeight);
    }
    ...
    if (ubCount > 0) {
      ...
      VSurfaceBlitBufToBuf(vsFB, vsSB, (UINT16)DestRect.iLeft, (UINT16)DestRect.iTop, sWidth,
                           sHeight);
    }
  } else {
    // Restore the old rect
