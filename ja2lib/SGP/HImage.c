...
HIMAGE CreateImage(const char *ImageFile, UINT16 fContents) {
  ...
  if (StrPtr == NULL) {
    ...
    DebugMsg(TOPIC_HIMAGE, DBG_LEVEL_2, "No extension given, using default");
  ...
  if (!File_Exists(imageFileCopy)) {
    DebugMsg(TOPIC_HIMAGE, DBG_LEVEL_2, String("Resource file %s does not exist.", imageFileCopy));
    return (NULL);
  }
...
struct SGPPaletteEntry *ConvertRGBToPaletteEntry(UINT8 sbStart, UINT8 sbEnd, UINT8 *pOldPalette) {
  ...
  DebugMsg(TOPIC_HIMAGE, DBG_LEVEL_0, "Converting RGB palette to struct SGPPaletteEntry");
