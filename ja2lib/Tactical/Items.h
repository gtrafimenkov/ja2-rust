...
extern int8_t FindObjClassAfterSlot(struct SOLDIERTYPE *pSoldier, int8_t bStart, uint32_t usItemClass);
...
extern uint16_t UseKitPoints(struct OBJECTTYPE *pObj, uint16_t usPoints, struct SOLDIERTYPE *pSoldier);
...
extern BOOLEAN CreateItems(uint16_t usItem, int8_t bStatus, uint8_t ubNumber, struct OBJECTTYPE *pObj);
