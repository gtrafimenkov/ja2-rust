...
BOOLEAN SaveStrategicMovementGroupsToSaveGameFile(FileID hFile) {
  ...
  File_Write(hFile, &uiNumberOfGroups, sizeof(UINT32), &uiNumBytesWritten);
  ...
  File_Write(hFile, uniqueIDMask, sizeof(UINT32) * 8, &uiNumBytesWritten);
...
BOOLEAN LoadStrategicMovementGroupsFromSavedGameFile(FileID hFile) {
  ...
  File_Read(hFile, &uiNumberOfGroups, sizeof(UINT32), &uiNumBytesRead);
  ...
  File_Read(hFile, uniqueIDMask, sizeof(UINT32) * 8, &uiNumBytesRead);
...
BOOLEAN SavePlayerGroupList(FileID hFile, struct GROUP *pGroup) {
  ...
  File_Write(hFile, &uiNumberOfNodesInList, sizeof(UINT32), &uiNumBytesWritten);
  if (uiNumBytesWritten != sizeof(UINT32)) {
    // Error Writing size of L.L. to disk
    return (FALSE);
  }
  ...
  while (pTemp) {
    ...
    File_Write(hFile, &uiProfileID, sizeof(UINT32), &uiNumBytesWritten);
    if (uiNumBytesWritten != sizeof(UINT32)) {
      // Error Writing size of L.L. to disk
      return (FALSE);
    }
...
BOOLEAN LoadPlayerGroupList(FileID hFile, struct GROUP **pGroup) {
  ...
  File_Read(hFile, &uiNumberOfNodes, sizeof(UINT32), &uiNumBytesRead);
  if (uiNumBytesRead != sizeof(UINT32)) {
    // Error Writing size of L.L. to disk
    return (FALSE);
  }
  ...
  for (cnt = 0; cnt < uiNumberOfNodes; cnt++) {
    ...
    File_Read(hFile, &uiProfileID, sizeof(UINT32), &uiNumBytesRead);
    if (uiNumBytesRead != sizeof(UINT32)) {
      // Error Writing size of L.L. to disk
      return (FALSE);
    }
...
BOOLEAN SaveWayPointList(FileID hFile, struct GROUP *pGroup) {
  ...
  File_Write(hFile, &uiNumberOfWayPoints, sizeof(UINT32), &uiNumBytesWritten);
  if (uiNumBytesWritten != sizeof(UINT32)) {
    // Error Writing size of L.L. to disk
    return (FALSE);
  }
...
BOOLEAN LoadWayPointList(FileID hFile, struct GROUP *pGroup) {
  ...
  File_Read(hFile, &uiNumberOfWayPoints, sizeof(UINT32), &uiNumBytesRead);
  if (uiNumBytesRead != sizeof(UINT32)) {
    // Error Writing size of L.L. to disk
    return (FALSE);
  }
