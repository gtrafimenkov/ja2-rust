  ...
  while (pNode) {
    ...
    if ((iDirection != -1)) {
      if ((!fZoomFlag) ||
        ...
        BltVideoObject2(vsFB, hMapHandle, (UINT16)iDirection, iX, iY, VO_BLT_SRCTRANSPARENCY, NULL);
...
void RenderIconsPerSectorForSelectedTown(void) {
  ...
  for (iCounter = 0; iCounter < 9; iCounter++) {
    ...
    for (iCurrentTroopIcon = 0; iCurrentTroopIcon < iTotalNumberOfTroops; iCurrentTroopIcon++) {
      ...
      BltVideoObject2(vsFB, hVObject, (UINT16)(iCurrentIcon), sX, sY, VO_BLT_SRCTRANSPARENCY, NULL);
    }
  }
