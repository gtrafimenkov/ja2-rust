...
void DrawCharacterInfo(INT16 sCharNumber) {
  ...
  // what kind of merc
    ...
    INT32 iBeenHiredFor = (GetGameTimeInMin() / NUM_MIN_IN_DAY) - pSoldier->iStartContractTime;
...
void HandleShadingOfLinesForContractMenu(void) {
  ...
  if ((pSoldier->ubWhatKindOfMercAmI == MERC_TYPE__AIM_MERC) && (pSoldier->bLife >= OKLIFE)) {
    ...
    // one week
    if ((INT32)(pProfile->uiWeeklySalary) > LaptopMoneyGetBalance()) {
    ...
    // two weeks
    if ((INT32)(pProfile->uiBiWeeklySalary) > LaptopMoneyGetBalance()) {
...
BOOLEAN AnyMercsLeavingRealSoon() {
  UINT32 uiCounter = 0;
  UINT32 uiTimeInMin = GetGameTimeInMin();
