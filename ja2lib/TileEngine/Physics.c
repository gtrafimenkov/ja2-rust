...
void ObjectHitWindow(INT16 sGridNo, UINT16 usStructureID, BOOLEAN fBlowWindowSouth,
                     BOOLEAN fLargeForce);
FLOAT CalculateObjectTrajectory(INT16 sTargetZ, struct OBJECTTYPE *pItem, vector_3 *vPosition,
...
vector_3 FindBestForceForTrajectory(INT16 sSrcGridNo, INT16 sGridNo, INT16 sStartZ, INT16 sEndZ,
                                    real dzDegrees, struct OBJECTTYPE *pItem, INT16 *psGridNo,
                                    FLOAT *pzMagForce);
  ...
  if (pObject->fVisible) {
    if (pObject->Obj.usItem != MORTAR_SHELL || pObject->ubActionCode != THROW_ARM_ITEM) {
      if (pObject->pNode != NULL) {
        ...
        pObject->pNode->sRelativeX = (INT16)pObject->Position.x;  // + pTrav->x_offset;
        pObject->pNode->sRelativeY = (INT16)pObject->Position.y;  // + pTrav->y_offset;
        pObject->pNode->sRelativeZ =
            (INT16)CONVERT_HEIGHTUNITS_TO_PIXELS((INT16)pObject->Position.z);

        // Update position data
        pObject->pShadow->sRelativeX = (INT16)pObject->Position.x;  // + pTrav->x_offset;
        pObject->pShadow->sRelativeY = (INT16)pObject->Position.y;  // + pTrav->y_offset;
...
BOOLEAN SavePhysicsTableToSaveGameFile(FileID hFile) {
  ...
  File_Write(hFile, &usPhysicsCount, sizeof(UINT32), &uiNumBytesWritten);
...
BOOLEAN LoadPhysicsTableFromSavedGameFile(FileID hFile) {
  ...
  File_Read(hFile, &guiNumObjectSlots, sizeof(UINT32), &uiNumBytesRead);
